    ...
    with open(gff3_file, "r") as gff, open(output_bed_file, "w") as bed:
        for line in gff:
            ...
            if parts[6] == "+":
                ...
                bed.write(
                    f"{parts[0]}\t{start}\t{end}\t{name}\t{parts[5]}\t{parts[6]}\n"
                )
    ...
    with open(output_file, "w") as file:
        ...
        for orf in orfs:
            ...
            if isinstance(in_gene, int):
                ...
                file.write(
                    f"{orf[0]}\t{orf[1]}\t{in_gene[0]}\t{in_gene[1]}\t{in_gene[2]}\n"
                )
...
def create_output_orf_forward_strand(
    ...
    for filename in os.listdir(input_orf_folder_path):
        ...
        if filename.endswith(ending):
            create_one_strand_orf_file(
                f"{input_orf_folder_path}/{filename}", output_folder_path
            )
